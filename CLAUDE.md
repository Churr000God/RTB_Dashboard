...
| Dashboard procesaba snapshots con datos incompletos tras reinicio de la Pi | Nextcloud necesitaba contrasena al arrancar → archivos de n8n llegaban escalonados → `wait_for_changed_cotizaciones` detectaba el primer CSV y lanzaba todo el pipeline sin esperar los demas | Flujo cambiado a manual: `actualizar-datos` ya no procesa; polling en frontend con `/api/estado-descarga`; usuario presiona Regenerar cuando llegan ≥ 20 archivos |
| Estado "Enviado" en Logistica muestra menos pedidos de los esperados | El CSV `Pedidos_Aprobados` filtra por **fecha de aprobacion** del periodo; pedidos aprobados antes del rango pero enviados dentro no aparecen | Es correcto por diseno. Para ver el backlog real completo revisar el CSV `Pedidos_Enviados` directamente y cruzar con `fecha_entrega` vacia |
...
| 2026-06-13 | Command Center: flujo de webhook en dos fases con `localStorage`. (1) webhook termina → guarda `{fecha_desde, fecha_hasta}` en `localStorage('rtb_csv_ready')` → `location.reload()`. (2) Al cargar la pagina el init detecta el flag → restaura fechas → mostraba banner → disparaba `autoRegenerateAfterWebhook(ctx)` automaticamente. Sustituido el 2026-06-15 (ver abajo). |
| 2026-06-15 | **Puerto cambiado a 8002.** `nexus-ops-rtb-backend-1` tambien usa 8000 — no pueden coexistir. El `docker-compose.yml` de produccion mapea `"8002:8000"`. El archivo del repo mantiene el mapeo relativo sin puerto fijo. |
| 2026-06-15 | `restart: unless-stopped` agregado a todos los servicios Docker de RTB_Dashboard y Nexus-Ops-RTB. Sin esto, al reiniciar la Raspberry Pi los contenedores no levantaban solos. |
